...
export async function completeWaterFetch(sessionId) {
  ...
  // Notify customer: session complete receipt
  ...
  if (newBalance < 500) {
    createSystemNotification({
      userId: session.user_id,
      type:   'low_balance',
      title:  '⚠️ Low card balance',
      body:   `Your balance is ${newBalance.toLocaleString()} RWF. Please recharge soon to continue using water services.`,
    }).catch(() => {});
  }
